...
def chunk_story(text, max_chunk_size=1000, overlap_size=150):
    ...
    print(f"[chunk_story] {len(chunked_with_overlap)} chunks produced")
...
def initialize_chroma_collection(chunks):
    ...
    try:
        ...
        print(f"[initialize_chroma_collection] Loaded existing collection with {collection.count()} chunks")
    except Exception:
        ...
        print(f"[initialize_chroma_collection] Created new collection with {len(chunks)} chunks")
...
def retrieve_chunks_from_chroma(question, collection, top_k=3):
    ...
    print(f"[search_story] \"{question[:60]}\" -> {len(retrieved_chunks)} chunks")
...
# Initialize story chunks and Chroma collection
chunks = chunk_story(story)
collection = initialize_chroma_collection(chunks)
print(f"[story_rag] Ready: {len(chunks)} chunks in Chroma collection\n")
