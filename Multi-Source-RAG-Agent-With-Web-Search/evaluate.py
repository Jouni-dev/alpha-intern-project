...
9 metrics aligned to Evaluating_AI_Agents_Primer, adapted from the Financial PDF
Tool's evaluator to cover this task's three tools: search_anthropic_info,
search_financial, and web_search.
"""
...
def score_exact_match_accuracy(expected: str, answer: str) -> float:
    ...
    for amount in expected_amounts:
        if amount in answer:
            print(f"[score_exact_match_accuracy] Found '{amount}' in answer OK")
            return 1.0
...
If the question is not about financial accounts at all, answer 'yes' (not applicable).
Answer with ONLY 'yes' or 'no'."""
...
def run_evaluation():
    ...
    anthropic_results = [r for r in results if "anthropic" in r["question_type"]]
    if anthropic_results:
        anthropic_faithfulness = sum(r["metrics"]["faithfulness"] for r in anthropic_results) / len(anthropic_results)
        scorecard["regression_check"]["anthropic_faithfulness"] = anthropic_faithfulness
        status = "PASS" if anthropic_faithfulness >= 0.95 else "FAIL"
        print(f"\nRegression check (Anthropic-doc questions): {anthropic_faithfulness:.3f} {status}")

    validation_results = [r for r in results if r["question_type"] == "anthropic-validation"]
    if validation_results:
        validation_tool_accuracy = sum(r["metrics"]["tool_accuracy"] for r in validation_results) / len(validation_results)
        scorecard["regression_check"]["validation_tool_accuracy"] = validation_tool_accuracy
        status = "PASS" if validation_tool_accuracy >= 0.5 else "FAIL"
        print(f"Validation check (web_search reached when expected): {validation_tool_accuracy:.3f} {status}")
